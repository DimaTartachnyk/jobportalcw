...
function JobListing({user, profileInfo, jobList, jobApplications, filterCategories}) {
    ...
    function handleFilter(getSectionID, getCurrentOption) {
        ...
        if (indexOfCurrentSection === -1) {
            cpyFilterParams = {
                ...cpyFilterParams, [getSectionID]: [getCurrentOption],
            };
        } else {
            const indexOfCurrentOption = cpyFilterParams[getSectionID].indexOf(getCurrentOption);
            if (indexOfCurrentOption === -1) cpyFilterParams[getSectionID].push(getCurrentOption); else cpyFilterParams[getSectionID].splice(indexOfCurrentOption, 1);
        }
    ...
    const filterMenus = filterMenuDataArray.map((item) => ({
        id: item.id, name: item.label, options: [...new Set(filterCategories.map((listItem) => listItem[item.id])),],
    }));
            ...
            <div className="flex items-baseline justify-between border-b border-gray-200 pb-6 pt-24">
                ...
                <div className="flex items-center">
                    {profileInfo?.role === "candidate" ? <Menubar>
                        {filterMenus.map((filterMenu) => (// eslint-disable-next-line react/jsx-key
                            <MenubarMenu>
                                <MenubarTrigger>{filterMenu.name}</MenubarTrigger>
                                <MenubarContent>
                                    {filterMenu.options.map((option, optionIdx) => (<MenubarItem
                                            key={optionIdx}
                        ...
                        <div className="container mx-auto p-0 space-y-8 ">
                            <div className="grid grid-cols-1 gap-x-4 gap-y-8 md:grid-cols-2 lg:grid-cols-3">
                                {jobList && jobList.length > 0 ? jobList.map((jobItem) => profileInfo?.role === "candidate" ? (// eslint-disable-next-line react/jsx-key
                                    ...
                                    <RecruiterJobCard profileInfo={profileInfo} jobItem={jobItem}
